...
public class MySqlDatabaseStructureService : IDatabaseStructureService
{
    private readonly Dictionary<string, Dictionary<string, DatabaseFieldData>> _fieldData = new();

    public MySqlDatabaseStructureService(string connectionString, string dbSchema, List<ColumnMetadataDefinition> metadata)
    {
        DataSchema = dbSchema;
...
", conn);
    ...
    {
        return new MySqlCommand(commandText, connection as MySqlConnection, transaction as MySqlTransaction);
    }
